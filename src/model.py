...
from src.layers import CategoricalDense, TransformerEmbeddingLayer
...
class TransForeCaster(tf.keras.Model):
    def __init__(
        ...
        **kwargs
        ...
        self.output_purchase = Sequential(
        ...
        self.log_sigma_purchase = self.add_weight(
            name='log_sigma_purchase',
            shape=(),
            initializer='zeros',
            trainable=True
        )
    ...
    def call(self, inputs):
        ...
        output = self._get_cross_category_embedding(user_input, portrait_input,
                                                    inter_category_embed)
        output_purchase = self.output_purchase(output)
        output_churn = self.output_churn(output)
        return {'output_purchase': output_purchase, 'output_churn': output_churn} 
