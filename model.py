...
class TransForeCaster(tf.keras.Model):
    ...
    def __init__(self,
        ...
        self.trans_enc = TransformerEmbeddingLayer(head_size=head_size,
                                                   num_heads=num_heads,
                                                   layer_num=mha_num,
                                                   ff_dim=ff_dim,
                                                   name='trans_enc')
        ...
        self.user_info_embed = CategoricalDense(self.vocab_size_dict,
        ...
        self.status_embed = Sequential(
            [Dense(dim, activation='relu') for dim in
             status_embed_ff_dims + [embedding_dim]], name='status_embed')

        self.output_head = Sequential(
            [Dense(dim) for dim in output_ff_dims] + [
    ...
    def get_inter_category_embedding(self, all_inputs):
        ...
        epsilon = K.random_normal(
            shape=(batch_size, categroy_size, embedding_size))
